...
export default class Datatable extends Component {
  ...
  getData( page, search, orderby, sort, filter ) {
    var url = "http://localhost.genetsis/rest/api/yuser?";
    if ( page > 0 ) {
      url += "&page="+page;
    }
    ...
    fetch(url)
      ...
      .then( 
        (result) => {
          this.setState({
            data: result.data,
            pages: parseInt(result.count/result.per_page),
            total: result.count,
            loading: false,
            prev: page > 1 ? true : false,
            next: page < parseInt(result.count/result.per_page),
          });
        },
        (error) => {
        }
      ...
      <div className="page">
        ...
        <div className="dataset">
          <div className="searchBox">
            ...
            <Link to='/' activeClassName="active" replace> Using Eloquent</Link>
          </div>
